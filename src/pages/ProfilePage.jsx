...
function ProfilePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`http://10.5.0.26:5000/users?username=${user.username}&password=${currentPassword}`);
      if (response.data.length > 0) {
        ...
        if (profilePhotoFile) {
          ...
          reader.onloadend = async () => {
            ...
            await axios.put(`http://10.5.0.26:5000/users/${updatedUser.id}`, updatedUser);
